...
/*   Updated: 2020/03/30 14:29:25 by abaisago         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		dbg_print_room(void *content, size_t size, unsigned pos, unsigned max)
{
	...
	room = content;
	ret = ft_printf("[%-5s %d,%d] ", room->name, room->coord.x, room->coord.y);
	if (room->link_list != NULL)
		ret += ft_printf("(%d) ", room->nlinks);
	if (pos == max)
		ret += ft_printf("\n");
	return (ret);
}
...
void	dbg_farm_print(t_farm *farm)
{
	...
	while (++i < farm->size)
	{
		...
		j = -1;
		/* ft_printf("|%d| .. ", *(unsigned*)room->link_list->head->content); */
		while (++j < room->nlinks)
			ft_printf("<%04u> ", room->links[j]);
		ft_putchar('\n');
	}
}
